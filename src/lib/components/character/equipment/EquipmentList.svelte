<script lang="ts">
	import {
		...
		removePotion,
		updateCurrency
	} from '$lib/stores/core/character.svelte';
	...
	function handleOnAddGear(event: Event) {
		event.preventDefault();
		addNewGear();
	}

	function handleOnAddPotion(event: Event) {
		event.preventDefault();
		addNewPotion();
	}
</script>
...
<div class="space-y-4">
	...
	<ParchmentCell title="Currency">
		<div class="flex items-center gap-4 mb-4">
			<span class="text-lg">
				{characterStore.equipment.currency.gp} gp
			</span>
	...
	<ParchmentCell title="Gear">
		<div class="space-y-2 mb-4">
			{#each characterStore.equipment.gear as item}
				<div
					class="flex items-center justify-between p-2 bg-yellow-50 rounded border border-yellow-200"
				>
					<span>{item}</span>
					<button class="text-red-500 hover:text-red-700" onclick={() => removeGearItem(item)}>
						×
					</button>
				</div>
			{/each}
		</div>
		<form onsubmit={handleOnAddGear} class="flex gap-2">
			<input
	...
	<ParchmentCell title="Potions">
		<div class="space-y-2 mb-4">
			{#each characterStore.equipment.potions as potion}
				<div
					class="flex items-center justify-between p-2 bg-yellow-50 rounded border border-yellow-200"
				>
					<span>{potion}</span>
					<button class="text-red-500 hover:text-red-700" onclick={() => removePotionItem(potion)}>
						×
					</button>
				</div>
			{/each}
		</div>
		<form onsubmit={handleOnAddPotion} class="flex gap-2">
			<input
